  ...
  import {
    ...
    basePhrase,
    getFastRead,
  } from "../../services/dashboard/servicesDash";
  ...
  const ProDashboard = ({ data, student }) => {
    ...
    const subjects = data?.subjectsPerformance?.length
      ? data?.subjectsPerformance
      : fallbackSubjects;
    const trendTendence = data?.trending?.tendence ?? "+";
    const trendProjection = data?.trending?.projection ?? "0%";
    const bigPercent = subjects.reduce((big, act) => {
      if (act.percent > big.percent) {
        return act;
      }
      return big;
    });
    const smallPercent = subjects.reduce((small, act) => {
      if (act.percent < small.percent) {
        return act;
      }
      return small;
    });

    const selecionarFraseUnica = (listaDeFrases, nomeDaMateria) => {
      const tamanhoNome = nomeDaMateria.length;
      const indice = tamanhoNome % listaDeFrases.length;
      return listaDeFrases[indice];
    };

    const verifyData = (subjects, currentSubject) => {
      if (currentSubject.totalSimulations <= 0) {
        return "Faça mais simulados para consolidar esta análise.";
      }
      if (currentSubject.percent === 100) {
        return "Sua média de acertos está impecável, continue assim!";
      }

      if (bigPercent.name === currentSubject.name) {
        return `Sua melhor média! Você superou sua taxa geral em ${currentSubject.percent - averageHitRate}%`;
      }
      if (smallPercent.name === currentSubject.name) {
        return "Sua menor média, revise os conceitos básicos novamente.";
      }
      if (currentSubject.percent < 40) {
        return `Atenção em ${currentSubject.name}: você errou ${100 - currentSubject.percent}% das questões desse bloco.`;
      }
      if (
        currentSubject.percent >= averageHitRate &&
        currentSubject.percent < averageHitRate + 5
      ) {
        return selecionarFraseUnica(basePhrase.naMedia, currentSubject.name);
      }
      if (currentSubject.percent >= averageHitRate + 5) {
        return selecionarFraseUnica(
          basePhrase.superandoMedia,
          currentSubject.name,
        );
      }
      return selecionarFraseUnica(basePhrase.estavel, currentSubject.name);
    };

    useEffect(() => {
      const temporizador = setInterval(() => {
        setCount((last) => (last >= 4 ? 0 : last + 1));
      }, 10000);

      return () => clearInterval(temporizador);
    }, []);

    const fastRead = getFastRead({
      timeStudy,
      totalSimulations,
      trendProjection,
      trendTendence,
      isPositive: trendTendence === "+",
      bigPercent,
      smallPercent,
    });
    const IconFastRead = fastRead[count].icon;
    return (
      <div className="space-y-5 md:space-y-6 xl:space-y-6 2xl:space-y-8">
          ...
          <div className="relative z-10 grid gap-5 xl:grid-cols-[1fr_360px] xl:items-center 2xl:grid-cols-[1fr_420px] 2xl:gap-8">
            <div className="max-w-3xl">
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-500 md:text-base md:leading-7 2xl:text-base 2xl:leading-7">
                Seu painel Pro está atualizado. Continue treinando com foco total,
                sem distrações e com relatórios detalhados por disciplina.
              </p>
        ...
        <section className="grid gap-5 xl:grid-cols-[1fr_380px] 2xl:grid-cols-[1fr_460px] 2xl:gap-8">
          <div className="rounded-3xl border border-blue-100 bg-white p-5 shadow-sm md:rounded-4xl md:p-6 2xl:p-7">
            ...
            {/* Caixa container com altura máxima de 450px e barra de rolagem vertical */}
            <div className="mt-6 max-h-160 overflow-y-auto pr-2 space-y-4 scrollbar-thin scrollbar-thumb-blue-200 scrollbar-track-transparent">
              {subjects.map((subject) => {
                const style = proIcons[subject.name] || defaultStyle;
                  ...
                  >
                    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                      <div className="flex items-center gap-3">
                        <div
                          className={`grid size-11 shrink-0 place-items-center rounded-2xl ${style.bg} ${style.text}`}
                        >
                        ...
                        <div>
                          ...
                          <p className="mt-1 text-xs text-slate-500">
                            {subject.totalSimulations
                              ? verifyData(subjects, subject)
                              : `${subject.totalSimulations} simulados feitos`}
                          </p>
                        </div>
                      </div>

                      <span
                        className={`rounded-full bg-blue-50 px-3 py-1 text-xs font-bold ${style.text}`}
                      >
                        {subject.percent}%
                      </span>
                    </div>
          ...
          <aside className="space-y-5 md:space-y-6">
            <article className="rounded-3xl border border-blue-100 bg-white p-5 shadow-sm md:rounded-4xl 2xl:p-6">
              ...
              <div className="mt-5 flex items-center gap-4 rounded-2xl bg-blue-50/70 p-3 px-4 text-sm font-bold text-slate-700">
                <IconFastRead className="size-5" />
                {fastRead[count].text}
              </div>
            </article>

            <article className="rounded-3xl border border-blue-100 bg-linear-to-br from-blue-800 to-green-500 p-5 text-white shadow-sm md:rounded-4xl 2xl:p-6">
        ...
        <section className="rounded-3xl border border-blue-100 bg-white p-5 shadow-sm md:rounded-4xl md:p-6 2xl:p-7">
          ...
          <div className="mt-6 grid gap-3 lg:grid-cols-3">
            {allSimulations.length > 0 ? (
              allSimulations.slice(0, 3).map((simulation) => (
                <article
